...
=============================================================
  ...
  functions for reading CIF file into "struct BLOCK".
...
void   read_CIF_file_into_WORDs();
void   Translate_Lines_into_WORDs();
void   translate_CIF_WORDs_into_BLOCK_data();
...
void   make_rowline_from_temp_head_word_for_one_row_table();
...
void read_CIF_file_into_BLOCK(ifname,blk)
  char *ifname;
  struct BLOCK *blk;
{
  struct WORD HeadWord; 

  /* printf("#read_CIF_file_into_BLOCK('%s')\n",ifname); */
  read_CIF_file_into_WORDs(ifname,&HeadWord);
  ...
  /* printf("#Ntable %d\n",blk->Ntable); */
  free_WORDs(&HeadWord);
} /* end of read_CIF_file_into_BLOCK() */




void read_CIF_file_into_WORDs(ifname,HeadWord)
  char *ifname;
  struct WORD *HeadWord;
{
  FILE *fp;
  ...
  /* printf("#read_CIF_file_into_WORDs('%s')\n",ifname); */
...
} /* end of read_CIF_file_into_WORDs() */


void Translate_Lines_into_WORDs(HeadLineNode,HeadWord)
  struct LINENODE *HeadLineNode;
  struct WORD *HeadWord;
{
  char line[MAX_LENGTH_WORD],word[MAX_LENGTH_WORD], command[MAX_LENGTH_LINE];
  char quot_state;   /* state of quotation. ';':semicolon, 's'ingle('), 'd'ouble(")  */
  int  i,L,Lline,Lword;  
  struct WORD *wn;
  struct LINENODE *ln;

  /* printf("#Translate_Lines_into_WORDs()\n"); */
 ...
 */
    ...
    Lline = strip_change_line_char(ln->line);
    sprintf(line,"%s",ln->line);
    /* printf("line '%s' quot_state '%c'\n",line,quot_state); */

    if ((Lline>0) && (ln->line[0]!='#')){
...
} /* end of Translate_Lines_into_WORDs() */



void translate_CIF_WORDs_into_BLOCK_data(HeadWord,blk)
  struct WORD *HeadWord;
  struct BLOCK *blk;
{
/*
>> example of CIF lines << 
data_1UBQ
#
...
*/
  ...
  /* printf("#translate_CIF_WORDs_into_BLOCK_data()\n"); */
...
} /* end of translate_CIF_WORDs_into_BLOCK_data() */
  ...
  char *str;
{
  ...
  L = strlen(str);
  if ((L>0)&&(str[L-1]=='\n')) {str[L-1] = '\0'; L -= 1;}
  if ((L>0)&&(str[L-1]=='\r')) {str[L-1] = '\0'; L -= 1;}
  if ((L>0)&&(str[L-1]=='\n')) {str[L-1] = '\0'; L -= 1;}
  if ((L>0)&&(str[L-1]=='\r')) {str[L-1] = '\0'; L -= 1;}
  return(L);
}
  ...
  char *column_name;
{
  ...
  if ((c<0)||(c>tb->Ncolumn)){ 
    printf("#WARNING: no column for column_name '%s'",column_name);
    return("");
  }
