...
def metric_MDS(N,dmat,xpos,ypos,zpos,Sim2Dis='F'):
  ...
  for i in range(5):
    print "lambda%d %f"%(i,Bmat[i][i]) 

  index = [i for i in range(N)] 
  sindex = sorted(index,lambda x,y:cmp(Bmat[y][y],Bmat[x][x]))
  for ii in range(5):
    i = sindex[ii] 
    print ">slambda%d %f"%(i,Bmat[i][i]) 
  ...
  if (Bmat[j0][j0]>0.0):
    sqrtLamX = math.sqrt(Bmat[j0][j0])
  if (Bmat[j1][j1]>0.0):
    sqrtLamY = math.sqrt(Bmat[j1][j1])
  if (Bmat[j2][j2]>0.0):
    sqrtLamZ = math.sqrt(Bmat[j2][j2])
 
  for i in range(N):
    xpos[i] = sqrtLamX*Vmat[i][j0] 
    ypos[i] = sqrtLamY*Vmat[i][j1] 
    zpos[i] = sqrtLamZ*Vmat[i][j2] 
